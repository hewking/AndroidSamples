...
public class GreenDaoCompatibleUpdateHelper {
  ...
  public void compatibleUpdate(SQLiteDatabase sqliteDatabase,
      Class<? extends AbstractDao<?, ?>>... daoClasses) {

    if (daoClasses == null && callBack != null) {
      callBack.onFailedLog("daoClasses is null");
      callBack = null;
      return;
    }
    ...
    if (!generateTempTables_withExchangeDataFromOldTable(db,
        daoClasses))    /** 创建中间表 & 把旧表的数据迁移到中间表 */ {
      return;
    }
    ...
    restoreData_fromTempTableToNewTable(db,
        daoClasses);                     /** 把中间表的数据迁移到新表 & 删除中间表 */
    if (callBack != null) {
  ...
  @SuppressWarnings("all")
  public void compatibleUpdate(StandardDatabase db,
      Class<? extends AbstractDao<?, ?>>... daoClasses) {
  ...
  @SafeVarargs
  private static boolean generateNewTablesIfNotExists_withNoExchangeData(StandardDatabase db,
      Class<? extends AbstractDao<?, ?>>... daoClasses) {
    return reflectMethod(db, "createTable", true, daoClasses);
  }

  @SafeVarargs
  private static boolean generateTempTables_withExchangeDataFromOldTable(StandardDatabase db,
      Class<? extends AbstractDao<?, ?>>... daoClasses) {
    try {
      ...
      if (callBack != null) {
        callBack
            .onFailedLog("generateTempTables_withExchangeDataFromOldTable ===> " + e.toString());
      }
    }
    return false;
  }
  ...
  @SafeVarargs
  private static boolean dropAllTables(StandardDatabase db, boolean ifExists,
      @NonNull Class<? extends AbstractDao<?, ?>>... daoClasses) {
    return reflectMethod(db, "dropTable", ifExists, daoClasses);
  }

  @SafeVarargs
  private static boolean createAllTables_withNoExchangeData(StandardDatabase db,
      boolean ifNotExists, @NonNull Class<? extends AbstractDao<?, ?>>... daoClasses) {
   ...
   */
  @SafeVarargs
  private static boolean reflectMethod(StandardDatabase db, String methodName, boolean isExists,
      @NonNull Class<? extends AbstractDao<?, ?>>... daoClasses) {
   ...
   */
  @SafeVarargs
  private static void restoreData_fromTempTableToNewTable(StandardDatabase db,
      Class<? extends AbstractDao<?, ?>>... daoClasses) {
    try {
      for (int i = 0; i < daoClasses.length; i++) {
        DaoConfig daoConfig = new DaoConfig(db, daoClasses[i]);
...
//                    }
          ...
          if (!columns.contains(columnName)) {
            ...
            insertTableStringBuilder
                .append("ALTER TABLE " + tempTableName + " ADD COLUMN " + columnName +
